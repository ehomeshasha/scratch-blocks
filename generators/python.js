...
Blockly.Python.init = function(a) {
  ...
  for (var c = 0; c < a.length; c++)
    b[c] = Blockly.Python.variableDB_.getName(a[c].name, Blockly.Variables.NAME_TYPE) + " = None";
...
Blockly.Python.getAdjustedInt = function(a, b, c, d) {
  ...
  Blockly.isNumber(a) ? (a = parseInt(a, 10) + c,
  d && (a = -a)) : (a = 0 < c ? "int(" + a + " + " + c + ")" : 0 > c ? "int(" + a + " - " + -c + ")" : "int(" + a + ")",
  d && (a = "-" + a));
  return a
}
;
...
Blockly.Python.controls_if = function(a) {
  ...
  do {
    ...
    c += (0 == b ? "if " : "elif ") + d + ":\n" + e;
    ++b
  } while (a.getInput("IF" + b));a.getInput("ELSE") && (e = Blockly.Python.statementToCode(a, "ELSE") || Blockly.Python.PASS,
    c += "else:\n" + e);
  return c
}
;
...
Blockly.Python.procedures_defreturn = function(a) {
  for (var b = [], c, d = a.workspace.getAllVariables() || [], e = 0; c = d[e]; e++)
    c = c.name,
    -1 == a.arguments_.indexOf(c) && b.push(Blockly.Python.variableDB_.getName(c, Blockly.Variables.NAME_TYPE));
  ...
  Blockly.Python.STATEMENT_PREFIX && (c = Blockly.Python.prefixLines(Blockly.Python.STATEMENT_PREFIX.replace(/%1/g, "'" + a.id + "'"), Blockly.Python.INDENT) + c);
  ...
  for (var g = [], e = 0; e < a.arguments_.length; e++)
    g[e] = Blockly.Python.variableDB_.getName(a.arguments_[e], Blockly.Variables.NAME_TYPE);
  b = "def " + d + "(" + g.join(", ") + "):\n" + b + c + f;
